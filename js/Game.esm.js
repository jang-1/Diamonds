...
import { DATALOAED_EVENT_NAME } from './Loader.esm.js';
...
class Game extends Common {
	...
	checkPosibilityMovement() {
		...
		this.isPossibleToMove = this.gameState.getGameBoard().some((diamond, index, diamonds) => {
			...
			if(
				index % DIAMONDS_ARRAY_WIDTH < DIAMONDS_ARRAY_WIDTH - 3 && diamond.kind === diamonds[index + 2].kind 
			...
			//move left =>check if is on the top of the colum
			...
			//move left => check if is on the bottom
			if(
				index % DIAMONDS_ARRAY_WIDTH
				&& Math.floor(index / DIAMONDS_ARRAY_WIDTH) < DIAMONDS_ARRAY_HEIGHT - 2
				&& diamond.kind === diamonds[index - DIAMONDS_ARRAY_WIDTH - 1].kind
			...
			if(
				index % DIAMONDS_ARRAY_WIDTH
				&& index % DIAMONDS_ARRAY_WIDTH > DIAMONDS_ARRAY_WIDTH - 1 && Math.floor(index / DIAMONDS_ARRAY_WIDTH) < DIAMONDS_ARRAY_HEIGHT - 1
				&& diamond.kind === diamonds[index + DIAMONDS_ARRAY_WIDTH + 1].kind
				&& diamond.kind === diamonds[index + DIAMONDS_ARRAY_WIDTH - 1].kind
			) {
				return true;
			...
			if(
				Math.floor(index / DIAMONDS_ARRAY_WIDTH) > 3 && diamond.kind === diamonds[index - DIAMONDS_ARRAY_WIDTH * 2].kind
				&& diamond.kind === diamonds[index - DIAMONDS_ARRAY_WIDTH * 3].kind
			) {
				return true;
			}
